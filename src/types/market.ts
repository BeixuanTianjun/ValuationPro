...
export interface StrategyProfile {
  ...
  tagline: string;
  description: string;
  /** Weights over the normalised factor blocks; may be negative. */
  weights: Record<string, number>;
  /** Filter overrides layered on top of the user's screen settings. */
  overrides?: Partial<ScreenFilters>;
}

export interface ScreenFilters {
  minClose: number;             // IDR — keeps out the gocap / auto-reject zone
  maxClose: number;
  minMedianValueIdrBn: number;  // 20d median turnover
  minMarketCapIdrBn: number;
  minTradedSessions20: number;
  minSessionsAvailable: number;
  excludeBoards: string[];      // e.g. ["Acceleration"]
  sectors: string[];            // empty = all
  indexFilter: string;          // '' = whole market, else an index code
  maxPicks: number;
}
...
export interface TradePlan {
  ...
  suggestedShares: number;      // sized to riskBudget, capped at 10% of ADV
  suggestedLots: number;
  positionValueIdr: number;
  liquidityCappedLots: number;
}
